...
require File.join("..",File.dirname(__FILE__),"lib/fastersam/library")

path = File.expand_path(File.dirname(__FILE__))
ext = FasterSam::Library.lib_extension

flags = ""
compile = ""
if ext == "so" then
  flags = "-shared -Wl,-soname,libfastersam.so"
  compile = " -fPIC"
...
task :compile_lib => OBJ_SRC do
  sh "gcc #{flags} -std=c99 "+OBJ_SRC.join(" ")+" -o libfastersam.#{ext}"
end
