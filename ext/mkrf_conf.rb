...
require File.join("..",File.dirname(__FILE__),"lib/bettersam/library")

path = File.expand_path(File.dirname(__FILE__))
ext = BetterSam::Library.lib_extension

flags = ""
compile = ""
if ext == "so" then
  flags = "-shared -Wl,-soname,libbettersam.so"
  compile = " -fPIC"
...
task :compile_lib => OBJ_SRC do
  sh "gcc #{flags} -std=c99 "+OBJ_SRC.join(" ") + " -o libbettersam.#{ext}"
end
